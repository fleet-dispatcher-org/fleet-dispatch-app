...
export default function LoadStatusClient({ routeId, initialStatus }: Props) {
    const [status, setStatus] = useState(initialStatus);

    async function updateLoadStatus(routeId: string, newStatus: string) {
        try {
    ...
    <div className="inline-block bg-gray-900 border border-gray-700 rounded-lg p-4 hover:bg-gray-700 transition-colors">
        <div className="flex flex-col space-y-2">
            ...
            <select 
                ...
                onChange={(e) => updateLoadStatus(routeId, e.target.value)}>
