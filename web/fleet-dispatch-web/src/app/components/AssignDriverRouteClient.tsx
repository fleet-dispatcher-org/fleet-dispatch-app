...
export default function AssignDriverClient({  routeId, assignedDriver }: AssignDriverClientProps) {
    ...
    const handleAssignDriver = async (assignedDriver: string) => {
        try {
            const [userRoutesResponse, loadResponse] = await Promise.all([
                fetch(`/api/dispatcher/routess/byUser/${assignedDriver}`, {
                    method: "GET",
                    headers: {
            ...
            const allRoutes = await userRoutesResponse.json();
            const filteredRoutes = allRoutes.filter((route: Route) => route.assigned_driver == assignedDriver && route.status == "SUGGESTED");
            filteredRoutes.forEach((route: Route) => {
                fetch(`/api/dispatcher/routes/${route.id}` , {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
                    },
