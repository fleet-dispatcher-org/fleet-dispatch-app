...
export default function AcceptDenyButtons({ routeId, driverId, trailerId, truckId }: AcceptDenyButtonsProps) {
    const session = useSession().data;
    const handleAccept = async (routeId: string,) => {
    try {
        // Execute all requests concurrently and get responses
        const [routeResponse, driverResponse, truckResponse, trailerResponse] = await Promise.all([
        ...
        if (!routeResponse.ok) {
            ...
            throw new Error(`HTTP ${routeResponse.status}: ${errorData.message || 'Failed to update load'}`);
        }
    ...
    const handleDeny = async (routeId: string) => {
        try {
