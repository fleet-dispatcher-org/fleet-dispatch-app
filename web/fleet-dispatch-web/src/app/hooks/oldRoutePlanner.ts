...
//         console.log(`Optimized Path for Driver: ${optimized.map(n => n.location.address).join(' -> ')}`); 
...
//                 driverGroup,
//                 // I'll need to find a way to include the feasible start points
//                 availableLoads, 
//                 maxLoadsPerRoute, 
//                 maxAlternatives
//             );
...
//                 console.log(`Distance from ${driverGroup.driver.first_name} to load ${load.id} and pickup location ${load.origin} and destination ${load.destination}: ${distanceToPickup.toFixed(2)} km`);
...
//                     console.log(`FEASIBLE: Load ${load.id} is feasible for driver ${driverGroup.driver.first_name} based on this location: ${load.origin} + ${load.destination}`);
...
//     private findFeasibleRoutes(driverGroup: DriverGroup, feasibleLoads: Load[], unassignedLoads: Load[], maxDistance: number) {
//         const routes: RouteTree[] = [];
//         const nodes: RouteNode[] = [];
//         const driverCoords = driverGroup.driver.home_coordinates as Array<{lat: number, long: number}> | null;
        

//         for (const load of feasibleLoads) {
//             const originCoords = load.origin_coordinates as Array<{lat: number, long: number}> | null;
//             const originLat = originCoords?.[0]?.lat;
//             const originLong = originCoords?.[0]?.long;
//             const destinationCoords = load.destination_coordinates as Array<{lat: number, long: number}> | null;
//             const destinationLat = destinationCoords?.[0]?.lat;
//             const destinationLong = destinationCoords?.[0]?.long;
...
//                 nodeType: 'PICKUP',
//                 location: {
//                     coordinates: { lat: originLat!, long: originLong! },
//                     address: load.origin
//                 },
//                 serviceTime: 0,
//                 sequence: 0,
...
//             const first_delivery: RouteNode = {
//                 id: `DELIVERY_${load.id}`,
//                 loadId: load.id,
//                 load: load,
//                 nodeType: 'DELIVERY',
//                 location: {
//                     coordinates: { lat: destinationLat!, long: destinationLong! },
...
//             nodes.push(first_pickup, first_delivery);
...
//             let tempDestinationLat = destinationLat;
//             let tempDestinationLong = destinationLong;

//             for(const unassignedLoad of unassignedLoads) {
//                 const unassignedOriginCoords = unassignedLoad.origin_coordinates as Array<{lat: number, long: number}> | null;
//                 const unassignedOriginLat = unassignedOriginCoords?.[0]?.lat;
//                 const unassignedOriginLong = unassignedOriginCoords?.[0]?.long;
//                 const unassignedDestinationCoords = unassignedLoad.destination_coordinates as Array<{lat: number, long: number}> | null;
//                 const unassignedDestinationLat = unassignedDestinationCoords?.[0]?.lat;
//                 const unassignedDestinationLong = unassignedDestinationCoords?.[0]?.long;


//                 const compared_distance = this.calculateDistance(tempDestinationLat!, tempDestinationLong!, unassignedOriginLat!, unassignedOriginLong!);
//                     if (compared_distance <= maxDistance) {
                        

//                         const second_pickup: RouteNode = {
//                             id: `PICKUP_${unassignedLoad.id}`,
//                             loadId: unassignedLoad.id,
//                             load: unassignedLoad,
//                             nodeType: 'PICKUP',
//                             location: {
//                                 coordinates: { lat: unassignedOriginLat!, long: unassignedOriginLong! },
//                                 address: unassignedLoad.origin
//                             },
//                             serviceTime: 0,
//                             sequence: 0,
//                             childNodes: [],
...
//                         const second_delivery: RouteNode = {
//                             id: `DELIVERY_${unassignedLoad.id}`,
//                             loadId: unassignedLoad.id,
//                             load: unassignedLoad,
//                             nodeType: 'DELIVERY',
//                             location: {
//                                 coordinates: { lat: unassignedDestinationLat!, long: unassignedDestinationLong! },
//                                 address: unassignedLoad.destination
//                             },
//                             serviceTime: 0,
//                             sequence: 0,
//                             childNodes: [],
...
//                         tempDestinationLat = unassignedDestinationLat;
//                         tempDestinationLong = unassignedDestinationLong;

//                         nodes.push(second_pickup, second_delivery);
                        
//                     }
                
//                 route.routePath = nodes;
